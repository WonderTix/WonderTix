...
export const EventShowingView = (props: EventInstanceViewProps) => {
  ...
  const showingDate = new Date(`${toDateStringFormat(showing.eventdate)} ${showing.eventtime.slice(0, 8)}`);
      ...
      >
        <div className={'col-span-12 min-[1350px]:col-span-4 rounded-lg p-2 w-[100%] bg-gray-300'}>
          <div className={`flex flex-col justify-center bg-white m-auto col-span-12 min-[1350px]:col-span-4 rounded-lg p-3 w-[100%] h-[100%]`}>
            ...
            <LineItem
              label={'Date'}
              information={
                format(showingDate, 'eee, MMM dd yyyy')
              }
            />
            <LineItem
              label={'Time'}
              information=
                {format(showingDate, 'hh:mm a')}
            />
            <LineItem
              label={'Total Tickets'}
        ...
        <div className={'overflow-y-auto overflow-x-auto col-span-12 min-[1350px]:col-span-7 shadow-xl border border-white rounded-xl bg-white w-[100%] min-h-[100px]'}>
          <table className={'table table-fixed text-sm min-w-[100%]'}>
            <thead className={`text-left text-zinc-800 whitespace-nowrap bg-gray-300 ${showPopUp?'':'sticky'} top-0 `}>
              <tr>
        ...
        <div className={'grid content-center mx-auto col-span-12 min-[1350px]:col-span-1'}>
          <button
            ...
            type={'button'}
            onClick={setEdit}
            className={'bg-blue-600 hover:bg-blue-700  disabled:bg-gray-700 text-white font-bold p-2 px-4 rounded-xl'}
          >
      Edit
          </button>
        </div>
