...
import { EventInfo1, EventInfo2, EventInfo3, EventInfo4 } from './testData/ConstsPackage';
...
test('addNewEvents',async({page})=>{
  const eventsPage = new EventsPage(page);
  await eventsPage.goto();
  await eventsPage.addnewevent(EventInfo1);
});
...
test('addDeleteEvents',async({page})=>{
  ...
  await eventsPage.addnewevent(EventInfo2);
  ...
  await eventsPage.goToEventFromManage(EventInfo2.eventFullName);
  ...
  await eventsPage.deleteTheEvent(EventInfo2.eventFullName);
});
...
test.fixme('editEvents',async({page})=>{
  ...
  await eventsPage.editTheEventInfo(EventInfo3);
  //Search for the event by its new name
  await eventsPage.searchForEventByName(EventInfo3);
  //Search for the event by its new description
  await eventsPage.editTheEventInfo(EventInfo3);
  await eventsPage.searchForEventByDes(EventInfo3);
  //Now let's change everything back
  await eventsPage.editTheEventInfo(EventInfo4);
});
