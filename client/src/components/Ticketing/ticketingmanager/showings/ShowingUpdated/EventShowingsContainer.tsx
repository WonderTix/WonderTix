...
export const EventShowingsContainer = () => {
  const {token, setReloadShowing, showingData, eventData, editing, setPopUpProps} = useEvent();
  ...
  const getSortingFunction = () => {
    switch (sortBy) {
    case 1: return (a, b) => a.eventinstanceid-b.eventinstanceid;
    case 2: return (a, b) => a.totalseats-b.totalseats;
    case 3: return (a, b) => a.availableseats - b.availableseats;
    case 4: return (a, b) =>
      new Date(toDateStringFormat(b.eventdate)).getTime() -
        new Date(toDateStringFormat(a.eventdate)).getTime();
    case 5: return (a, b) => b.eventinstanceid-a.eventinstanceid;
    case 6: return (a, b) => b.totalseats-a.totalseats;
    case 7: return (a, b) => b.availableseats - a.availableseats;
    default: return (a, b) =>
      new Date(toDateStringFormat(a.eventdate)).getTime() -
        new Date(toDateStringFormat(b.eventdate)).getTime();
    }
  ...
  return (
    <div className='bg-white flex flex-col mt-6 p-6 rounded-xl shadow-xl text-zinc-800'>
      <div className={'grid grid-cols-12 p-2'}>
        <h2 className={'col-span-12 min-[650px]:col-span-6 text-xl md:text-2xl text-center min-[650px]:text-start pb-1'}>{eventData?.eventname} Showings</h2>
        <div className={'col-span-12 min-[650px]:col-span-6 flex flex-row justify-center min-[650px]:justify-end'}>
          <div className={'col-span-6 grid min-[1350px]:grid-cols-2 pr-1'}>
            <
              label htmlFor={'sortMethod'} className={'text-xs text-zinc-800 my-auto pr-2 text-center min-[1350px]:text-end'}>Sort By:</label>
            <select
              disabled={editing}
              value={sortBy}
          ...
          <button
            className={'bg-green-600 hover:bg-green-700 w-fit h-fit my-auto disabled:bg-gray-600 text-white p-2 font-bold rounded-xl col-span-6 flex flex-row'}
