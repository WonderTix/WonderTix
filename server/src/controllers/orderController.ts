    ...
    try {
        ...
        const toReturn = orders.map((order) => {
                ...
                .map((item) =>
                    item.singletickets
                        ...
                        .map((ticket) => {
                            ...
                            console.log(
                                ticket.eventtickets[0].eventinstances.events
                                    .eventname,
                            );
                            return ticket.eventtickets[0].eventinstances.events
                                .eventname;
                        }),
                )
                .flat()
            ...
            return {
                ...
                orderdate: `${orderdate.toString().slice(4, 6)}/${orderdate
                    .toString()
                    .slice(6, 8)}/${orderdate.toString().slice(0, 4)}`,
                ...evenythingElse,
                showings: names,
            };
        });
...
orderController.put('/refund/:id', async (req, res) => {
    try {
        ...
        if (!order.payment_intent) {
            return res
                .status(400)
                .json({ error: `Order ${orderID} is still processing` });
        }
        if (order.refund_intent) {
            return res
                .status(400)
                .json({ error: `Order ${orderID} has already been refunded` });
        }
