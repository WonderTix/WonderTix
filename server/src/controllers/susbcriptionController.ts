 ...
 *   get:
 *     summary: get subscription and related events in format needed for redemption
 *     tags:
 *     - Subscription API
 *     parameters:
...
subscriptionController.put(
    ...
    async (req: Request<{subscriptionid: number}, {}, SubscriptionTicketItemRequest>, res: Response) => {
      try {
        ...
        if (!subscription) {
          return res.status(400).json({error: `Subscription (${subscriptionId}) does not exist`});
        ...
        const eventInstances = await prisma.eventinstances.findMany({
          where: {
            eventinstanceid: {
              in: req.body.subscriptionticketitems.map((item) => +item.eventinstanceid),
            },
          },
          include: {
            ticketrestrictions: {
              where: {
                deletedat: null,
                ticketrestrictionsid: {
                  in: [...subscriptionTicketItemMap.keys()],
                },
              },
              include: {
                ticketitems: {
