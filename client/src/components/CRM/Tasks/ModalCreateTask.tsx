...
interface ModalCreateTaskProps {
  isVisible: boolean;
  onCancel: () => void;
  onSubmit: (createTask: TableRowType) => void;
}

const ModalCreateTask: React.FC<ModalCreateTaskProps> = ({
  onCancel,
  onSubmit,
  isVisible,
}: ModalCreateTaskProps): React.ReactElement | null => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const uniqueId = Math.max(...TABLE_ROWS.map((task) => task.id), 0) + 1;

    const createTask: TableRowType = {
      id: uniqueId,
