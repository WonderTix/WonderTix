...
async function seedPPHTransactions(prisma: PrismaClient) {
  ...
  try {
    ...
    if (transactionsCount > 0) {
      console.log('PPH_Transactions table already seeded.');
      return;
    }

    const yamlData = fs.readFileSync('./prisma/yaml-seeder-data/PPH_transactions.yaml', 'utf8');
    ...
    for (const item of data) {
      ...
      if (item.ticket_order_item_id) {
        const ticketOrderItemExists = await prisma.pphTicketorderitem.findUnique({
      ...
      if (canInsert) {
        await prisma.pphTransactions.create({
          data: {
...
module.exports = seedPPHTransactions;
