...
export class ContactPage {
  ...
  async checkCustomer(customer: CustomerInfo) {
    const currentCard = this.customerCard.filter({hasText: customer.email});
    expect(await currentCard.getByTestId('contact-name').textContent()).toBe(
      customer.fullName,
    );
    expect(await currentCard.getByTestId('contact-email').textContent()).toBe(
      customer.email,
    );
    expect(await currentCard.getByTestId('contact-address').textContent()).toBe(
      customer.streetAddress,
    );
    expect(await currentCard.getByTestId('contact-city').textContent()).toBe(
      customer.city,
    );
    expect(await currentCard.getByTestId('contact-state').textContent()).toBe(
      customer.state,
    );
    expect(await currentCard.getByTestId('contact-country').textContent()).toBe(
      customer.country,
    );
    expect(
      await currentCard.getByTestId('contact-postalCode').textContent(),
    ).toBe(customer.postCode);
    expect(await currentCard.getByTestId('contact-phone').textContent()).toBe(
      customer.phoneNumber,
    );
    expect(
      await currentCard.getByTestId('contact-accommodation').textContent(),
    ).toBe(customer.accommodations);
    expect(
      await currentCard.getByTestId('contact-comments').textContent(),
